...
class JungleBehavior {
    ...
    attackJungleCamp() {
        ...
        if (dist <= (hero.stats.attackRange || 100) + (creature.radius || 20)) {
    ...
    useJungleAbilities(creature) {
        ...
        const skillUsage = this.controller.getDifficultySetting?.('skillUsage') || 0.5;
    ...
    moveToCreature(creature) {
        ...
        const optimalRange = (hero.stats.attackRange || 100) * 0.9;
    ...
    shouldSecureObjective(creature) {
        ...
        if (hero.spell === 'smite' && (hero.spellCooldown || 0) <= 0) {
            return true;
        }
